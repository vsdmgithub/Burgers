...
! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   ...
   SUBROUTINE pre_analysis
    ...
    ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        !  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        IF ( ( dt .LT.  time_grid ) .AND. ( dt .LT. (0.05D0 * time_purging)) ) THEN
        ...
        ELSE
            ...
            WRITE(*,'(A50)'),'----------------------------------------------------------------------'
            WRITE(*,'(A50,F10.6)'),' RESET THE TIME STEP (AT MAX) AS :',MIN(time_grid,0.05D0 * time_purging)
    ...
    ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
        ...
        WRITE(233,"(A40)"),TRIM(ADJUSTL('----- PURGED   BURGERS  EQUATION----------------------'))
        WRITE(233,"(A40)"),TRIM(ADJUSTL(' - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -'))
        ...
        WRITE(233,"(A2,A20,A2,I5)"),'3.','Purged. Mode  ','= ',k_P
        WRITE(233,"(A2,A20,A2,ES12.5)")'4.','Time step   ','= ',dt
        WRITE(233,"(A2,A20,A2,2F5.2)")'5.','Purging params a,b ','= ',purging_alpha,purging_beta
        WRITE(233,"(A2,A20,A2,I8)")'6.',' Total time steps   ','= ',t_step_total
        WRITE(233,"(A2,A20,A2,F5.2)")'7.','Total time ','= ',time_total
        WRITE(233,"(A2,A20,A2,F8.4)")'8.','Purging time ','= ',time_purging
        WRITE(233,"(A2,A20,A2,I5)")'9.',' No of saves   ','= ',save_total
        WRITE(233,"(A2,A20,A2,F6.3)")'10.',' Initial energy ','= ',initial_en
    ...
    ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        IF (MOD(t_step,t_step_purging) .EQ. 0) THEN
                ...
                vel_k   =   purger  *   vel_k
                ! Velocity array is purged, when it is time.
        END IF
